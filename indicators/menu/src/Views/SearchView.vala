...
public class Slingshot.Widgets.SearchView : Gtk.ScrolledWindow {
    ...
    construct {
        ...
        var alert_view = new Gtk.Box(VERTICAL,3);
        var iconlol = new Gtk.Image.from_icon_name("image-missing", DIALOG);
		iconlol.pixel_size = 128;
        alert_view.pack_start(iconlol, true,false,95);
        alert_view.pack_start (new Gtk.Label ("No matches, try something else."), true, false, 0);
        alert_view.show_all ();
    ...
    private void update_header (SearchItem row, SearchItem? before) {
        ...
        var header = new Gtk.Label (row.result_type.to_string ());
